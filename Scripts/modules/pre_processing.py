...
raw=pd.read_csv(f"/Users/elyestabbane/Documents/UNICOG/2-Experiments/memocrush/Data/raw/experiment-2/{file_name}", names=col_names, header=None, index_col=False,delimiter=',',quotechar='"',skipinitialspace=True,encoding='utf-8')
...
matched_values = [complexities_post_fit_exp1.get(value, -1) for value in processed_clean['seq_name']]

processed_clean['LoT Complexity']=matched_values

# -- Adding values from subjective complexity experiment
matched_values_subj=[mean_subjective_complexity.get(value, -1) for value in processed_clean['seq_name']]
processed_clean['subjective_complexity']=matched_values_subj
...
#
# ---- Adding the columns relative to temporal structure
# 
holder_column=[]
holder_perfect=[]
for index, row in processed_clean.iterrows():
    holder_column.append(array_structure(row['sequences_response']))
processed_clean['response_structure']=holder_column

for index, row in processed_clean.iterrows():
    holder_perfect.append(row['response_structure']==row['sequences_structure'])
processed_clean['perfect_structure']=holder_perfect

# Adding a column for length of correct positions in a row
processed_clean['size_correct_chunk']=apply_correct_count(processed_clean)
...
processed_clean.to_csv(f"/Users/elyestabbane/Documents/UNICOG/2-Experiments/memocrush/Data/processed/experiment-2/processed_{file_name}", index=False)
...
Version: V1.5
Changes:
    - Adding columns: response_structure () and perfect_structure (is the structure correct ?)
    - What is the difference between 'comparable_temp' and 'response_structure' ?
        -- comparable_temp: compares shown sequence and response. It maps the response on the original ordinal positions.
            This allows to observe swaps and errors of ordinal information encoding.
            ex: original is ABC.ABC, response is BCA.BCA, comparable_temp is 120.120
            ex: original is ABC.ABC, response is CAB.CAB, comparable_temp is 201.201
            
        -- response_structure: captures the temporal structure of the response, independently of the sequence shown. It is insensitive to ordinal encoding errors.
            This allows to have a closer look at the participant's program and structure representation in memory.
            ex: original is ABC.ABC, response is BCA.BCA, response_structure is 012.012, perfect_structure is True
            ex: original is ABC.ABC, response is CAB.CAB, response_structure is 012.012, perfect_structure is True
            ex: original is ABC.ABC, response is ABC.ACB, response_structure is 012.021, perfect_structure is False
