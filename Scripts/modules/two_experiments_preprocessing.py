...
all_dictionary_complexities=[
    ...
    mean_revised_subjective_complexity,
    dict_shannon_entropy ,
    dict_shannon_entropy_bigram ,
    dict_lz_complexity,
...
# We also want one data set with only values from experiment 2 but for repetition sequences only
data_comp_ext_rep=data_comp[(data_comp['seq_name'].isin(seq_name_list_exp1_only))&(data_comp['which_exp']=='extension')]
data_comp_ext_rep.reset_index(drop=True,inplace=True)
data_comp_ext_rep.to_csv(os.path.join(save_path,f'{date}_complexity_dataset_EXT_only_REP.csv'), index=False)
...
Changes:
    - Adding column with mean estimated complexity ("subjective_complexity") from the online experiment.
